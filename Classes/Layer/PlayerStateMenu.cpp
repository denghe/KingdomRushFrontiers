...
bool PlayerStateMenu::init()
{
	...
	for(int i = 0; i < 6 ; i ++){
		...
		if(num > 0){
			stringForSkillFileName = __String::createWithFormat("backPack_icons_000%d.png",i+1)->getCString();
		}else{
			stringForSkillFileName = __String::createWithFormat("backPack_icons_off_000%d.png",i+1)->getCString();
		}
		...
		backPack_icons_Sprite[i]->setName(__String::createWithFormat(instance->SLOTX_SHOP,i)->getCString());
		backPackSprite->addChild(backPack_icons_Sprite[i],0);

		auto numLeft = Label::createWithTTF(__String::createWithFormat("%d",num)->getCString(),"arial.ttf",20);
	...
	skillListener->onTouchBegan = [&](Touch* touch, Event* event){
		...
		if (rect.containsPoint(locationInNode) && backPackSprite->isVisible()){  
			...
			if(num <= 0){
				...
				target->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat( "backPack_icons_off_00%02d.png",target->getTag()+1)->getCString()));
				...
				num--;
				static_cast<Label*>(target->getChildByTag(101))->setString(__String::createWithFormat("%d",num)->getCString());
...
{
	lifeLabel->setString(__String::createWithFormat("%d",life)->getCString());
}

void PlayerStateMenu::setGold(int gold)
{
	goldLabel->setString(__String::createWithFormat("%d",gold)->getCString());
}

void PlayerStateMenu::setWave(int thisWave,int totalWave)
{
	waveLabel->setString(__String::createWithFormat("WAVE %d/%d",thisWave,totalWave)->getCString());
}
