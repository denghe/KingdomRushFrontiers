...
void ShopScene::initBackpackIcon()
{
	...
	for(int i=0;i<6;i++){
		auto backPackIcon = Sprite::createWithSpriteFrameName(__String::createWithFormat("inaps_backPackIcons_000%d.png",i+1)->getCString());
		backPackIcon->setPosition(156,size.height*(0.72f-a[i]));
		addChild(backPackIcon,3);
		if(count[i] == 0)
			backPackIcon->setVisible(false);
		auto countLabel =  Label::createWithTTF(__String::createWithFormat("%d",count[i])->getCString(), "SohoGothicProMedium.ttf", 24); //小男孩的数量
...
//---------------------------------------------------------------------------------------------------------------------------//
	 //确定
	  ...
	  confirmButton->setCallback([&](Ref *pSender){
		...
		for(int i=0;i<6;i++)
		{
			UserDefault::getInstance()->setIntegerForKey(__String::createWithFormat(GameManager::getInstance()->SLOTX_SHOP,i)->getCString(),count[i]);		
		}
		Director::getInstance()->popScene();
	  });
