...
void Level4::addOrnament()
{
	...
	TreeSprite[3][0]->setPosition(Point(860,525));
...
void Level4::waveEvent()
{
	if(wave == 0){
		...
		for (int i = 1; i <= 9; i++)
		{
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("Stage4_ShipDoor_00%02d.png",i)->getCString());
			if(frame!=nullptr)
