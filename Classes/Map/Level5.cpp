...
void Level5::waveEvent()
{
	if(wave == 1){
		...
		for (int i = 1; i <= 36; i++)
		{
			frame1 = SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("Stage5_door_00%02d.png",i)->getCString());
			if(frame1!=nullptr)
...
void Level5::eyeEffect()
{
	...
	for (int i = 1; i <= 94; i++){
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("Stage5_door_eyes_00%02d.png",i)->getCString());
		if(frame!=nullptr)
...
void Level5::stonesEffect()
{
	...
	SpriteFrame* frame = NULL;
	for (int i = 1; i <= 94; i++){
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("Stage5_door_stones_00%02d.png",i)->getCString());
		if(frame!=nullptr)
		animFrames.pushBack(frame);
	}
