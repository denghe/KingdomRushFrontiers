...
bool BarrackSoldier::initSoldier(int level)
{
	...
	setLevel(level);
	setForce(3*level);
	baseSprite = Sprite::createWithSpriteFrameName(__String::createWithFormat("soldier_lvl%d_0001.png",level)->getCString());
	addChild(baseSprite);
	createAndSetHpBar();
...
void BarrackSoldier::update(float dt)
{
	...
	if(lastState!=getState()){
		...
		case(SoldierStateHit):{
			...
			stopSoldierAnimation();
			auto action = RepeatForever::create(Animate::create(AnimationCache::getInstance()->getAnimation(__String::createWithFormat("level%d_barracksSoilder_attack",level)->getCString())));
		...
		case(SoldierStateWait):{
			...
			stopSoldierAnimation();
			baseSprite->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("soldier_lvl%d_0001.png",level)->getCString()));}
			break;
	...
	if(nearestMonster!=NULL && nearestMonster->getCurrHp()>0){
		...
		if(SoldierHp == 0){
			...
			baseSprite->runAction(Sequence::create(
				Animate::create(AnimationCache::getInstance()->getAnimation(__String::createWithFormat("level%d_barracksSoilder_dead",level)->getCString())),
				FadeOut::create(1.0f)
				,NULL));
		}
	}else{
